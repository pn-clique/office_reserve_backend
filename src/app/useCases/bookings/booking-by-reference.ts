...
export class BookingByReferenceUseCase implements UseCase {
  async execute(request: string): Promise<HttpResponse<any>> {
    try {
      const booking = await Prisma.booking.findUnique({ where: { reference: request }, include: { user: true, place: true, modality: true, } });
      if(!booking) {
